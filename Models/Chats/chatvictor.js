...
document.getElementById('button-send').addEventListener('tap',function(){
    ...
    //victor开始发送消息  >> chentong
    // victor 用自己的名字作为 clientId，获取 IMClient 对象实例
	realtime.createIMClient('victor').then(function(victor) {
	  // 创建与Jerry之间的对话
	  return victor.createConversation({
	    members: ['chentong'],
	    name: 'victor chat with chentong',
	  });
	}).then(function(conversation) {
	  // MessageInputValue发送消息开始
	  return conversation.send(new AV.TextMessage(MessageInputValue));
	}).then(function(message) {
	  console.log('victor send message to chentong', 'SUCCESS！');
	}).catch(console.error);
...
// victor 登录, 实时监听chentong发送的消息  !!!!
realtime.createIMClient('victor').then(function(victor) {
  victor.on('message', function(message, conversation) {
    console.log('Message received: ' + message.text);
    
     //victor显示收到chentong消息的值message.text, 收到的消息放到左侧left
